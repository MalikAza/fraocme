...
class Day5(Solver):
    ...
    def part2(self, red_tiles: list[tuple[int, int]]) -> int:
        ...
        # grid the outline for display
        outline_grid = Grid.from_positions(outline, fill="*", default=".")
        self.debug(c.bold("\nOutline connecting red tiles:"))
        outline_translated = {(x - min_x, y - min_y) for x, y in outline}
        self.debug(
            lambda: print_grid(
                outline_grid,
                separator=" ",
                highlight=outline_translated,
                show_coords=True,
            )
        )
